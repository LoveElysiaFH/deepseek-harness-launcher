...
### 修复
- 控制台模式现在**总会弹出窗口**（此前若 dsh 已在运行会秒退、看起来像没弹窗）：
  - 已在运行且由本启动器启动 → 自动接管并重启进控制台
  - 已在运行但由外部启动（如手动 `npx`）→ 控制台显示说明并保持窗口，不误杀
- 修复：控制台模式下「保持窗口」用永不结束的定时器句柄维持事件循环
  （原先的空 Promise 不产生句柄，进程会直接退出）
- 测试：`resolveConsoleMode` 抽为纯函数，测试不再依赖本机配置文件
