...
Options:
  ...
  --console        Run dsh in a visible console window (close it to stop dsh)
...
If the harness lives in a non-standard location, point the launcher at it:
  dsh-launcher config set harness.cwd "D:/path/to/deepseek-harness"`,
    ...
    'server.consoleLabel': 'console',
    'server.consoleHint': 'Console mode: dsh runs in this window; close the window or press Ctrl+C to stop it.',
    'server.consoleForeign': 'Already running at {url}, but not started by this launcher. To run dsh in this console, stop that instance first (or use --port <n>). Closing this window does not affect it.',
...
选项:
  ...
  --console        在可见控制台窗口运行 dsh（关掉窗口即停止 dsh）
...
如果 harness 在非标准位置，请指定目录:
  dsh-launcher config set harness.cwd "D:/path/to/deepseek-harness"`,
    ...
    'server.consoleLabel': '控制台',
    'server.consoleHint': '控制台模式：dsh 在本窗口运行；关闭窗口或按 Ctrl+C 即可停止。',
    'server.consoleForeign': '{url} 已在运行，但不是本启动器启动的。想在本控制台运行 dsh，请先停止那个实例（或用 --port <n> 换端口）。关闭本窗口不会影响它。',
