...
>
> 控制台模式**总会弹窗**：若 dsh 已在运行，会**先停掉它再重启进控制台**（不管是谁启动的，
> 包括手动 `npx` 开的）。⚠️ 这等于强制重启，会终止正在运行的会话、未保存内容会丢失。
...
> 并短暂断开再重连。重启会停掉该端口上**任何**正在运行的 dsh 实例——不管是本启动器
> 启动的，还是你手动用 `npx @deepseek-ai/dsh web` 开的。
